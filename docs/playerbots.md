...
| Equipment and depots | Ordinary; restart and remote liquidation paths have focused evidence | Buys or equips supported upgrades, deposits retained loot, and can sell from local or remote depots. A Knight with an empty back slot can buy a standard backpack from a discovered shop. To upgrade a basic bag, it stages the complete bag in a discovered depot, buys with the empty back slot as the verified receipt destination, then retrieves the old bag into the new backpack. The bot stages only after its bank balance can pay the backpack price and preserve the configured reserve; carried coins inside the bag do not count for this preflight. Bounded failures restore the bag before purchase when possible. An unresolved depot recovery is stored on the player and tagged bag, stops the controller with an explicit failure, and resumes before normal goals after reload. If death equips a replacement basic bag, recovery deposits that complete bag before restoring the tagged original. Other containers and two-handed loadout trade-offs are unsupported and protected. |
