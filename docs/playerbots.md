...
| Navigation | Ordinary; transition and recovery scenarios have focused evidence | Routes across the loaded map using a shared coarse topology, then plans and executes local paths and supported transitions. Supports tools, doors, floor changes, teleports, and registered NPC travel. Explicitly blocked transition approaches are excluded so local routing can select another approach without discarding the destination. Exhausting a local search budget does not by itself trigger paid travel, and fare or safety rejection counts toward bounded no-progress recovery. Detailed search and recovery have budgets; unsupported transitions and changing obstacles can still prevent a journey. Static topology changes may require a supported reload or restart. |
