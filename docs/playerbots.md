...
| Hunting | Ordinary; map-derived selection has focused real-map coverage | Scores loaded hostile spawns against equipment, skills, health, supplies, route safety, expected XP, and cash pressure. Safe completed outings calibrate potion demand across monotonic level, skill, and maximum-resource improvements. Cheaper observations are not reused by weaker capabilities or across equipment changes. Food-backed recovery remains capability-specific unless the outing shows that recovery did not use food. Unsafe outings can raise demand but cannot lower it. Adaptive challenge and observations reset with the controller; no long progression soak is established. |
...
Supply calibration is deliberately gradual. A zero observed potion rate requires at least three safe samples and a projected remainder below one potion; three outings do not guarantee a zero estimate.
