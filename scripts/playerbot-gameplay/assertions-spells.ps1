function Assert-SpellTrainingEvents {
	...
	$lightAtReserve = @($events | Where-Object {
		$_.event -eq "spell_candidate" -and $_.spell -eq "Light" -and $_.result -eq "feasible" -and $_.price -eq 100
	})
	...
	if ($discovery.Count -lt 1 -or $selected.Count -ne 1 -or $lightAtReserve.Count -lt 1 -or $purchase.Count -ne 1 -or
		$completed.Count -ne 1 -or $terminal.Count -ne 0) {
		$lightCandidates = @($events | Where-Object { $_.event -eq "spell_candidate" -and $_.spell -eq "Light" } |
			ForEach-Object { "$($_.result)/$($_.reason)/reserve=$($_.reserve)" }) -join ","
		throw "Spell training failed. discovery=$($discovery.Count), selected=$($selected.Count), light_at_reserve=$($lightAtReserve.Count), purchases=$($purchase.Count), completed=$($completed.Count), terminal=$($terminal.Count), light=[$lightCandidates]."
	}
}
