...
local function addTravelKeyword(keyword, cost, destination, text, condition)
	...
	local travelKeyword = keywordHandler:addKeyword({keyword}, StdModule.say, {npcHandler = npcHandler, text = text or 'Do you seek a passage to ' .. titleCase(keyword) .. ' for |TRAVELCOST| gold?', cost = cost, discount = 'postman'})
