...
std::vector<playerbot::PlayerBotFixtureEvent> playerbot::PlayerBotFixtureDriver::runAdaptiveChallenge(Player& player)
{
	...
	for (const auto& [potions, duration] : {std::pair<uint32_t, uint32_t>{2, 900}, {10, 900}, {2, 1500}}) {
		PlayerBotHuntRuntime runtime(std::vector<Position>{});
		PlayerBotHuntRuntimePlanningInput input;
		input.player.health = input.player.maximumHealth = 200;
		input.player.potions = potions;
		input.huntDurationSeconds = duration;
		input.start.emplace();
		input.start->scan.candidateCount = 2;
		input.start->scan.candidateIndices = {0, 1};
		input.start->profile.currentHealth = 200;
		const auto now = std::chrono::steady_clock::now();
		runtime.advancePlanning(input, now);
		const auto work = runtime.advancePlanning(input, now);
		std::vector<PlayerBotHuntRuntimeScoreObservation> scores;
		for (const auto& candidate : work.scoreWork) {
			PlayerBotHuntRegion region;
			region.atlasVariantId = candidate.candidateIndex + 1;
			region.suitable = region.reachable = true;
			region.experiencePerMinute = candidate.candidateIndex == 0 ? 10 : 100;
			region.expectedDamagePerSecond = candidate.candidateIndex == 0 ? 0.1 : 1;
			region.combatFraction = 0.5;
			region.supplyProfile.potions = potions;
			region.supplyProfile.potionHealing = 125;
			region.reconcileTravel(duration, 0, 1);
			scores.push_back({candidate.candidateIndex, true, true, true, std::move(region)});
		}
		runtime.completeScoreWork(scores, 0);
		const auto selected = runtime.advancePlanning(input, now);
		std::ostringstream supply;
		supply << "\"source\":\"synthetic_runtime_candidates\",\"potions\":" << potions
		       << ",\"duration_seconds\":" << duration
		       << ",\"easy_expected_potions\":" << (scores.size() == 2 ? scores[0].region.supplyBudget.expectedPotions : -1)
		       << ",\"costly_expected_potions\":" << (scores.size() == 2 ? scores[1].region.supplyBudget.expectedPotions : -1)
		       << ",\"easy_budget_fits\":" << (scores.size() == 2 && scores[0].region.supplyBudget.fits ? "true" : "false")
		       << ",\"costly_budget_fits\":" << (scores.size() == 2 && scores[1].region.supplyBudget.fits ? "true" : "false")
		       << ",\"selection_rule\":" << jsonString(selected.selectedRegion ? playerBotHuntSelectionRule(*selected.selectedRegion) : "none")
		       << ",\"scored_candidates\":" << scores.size()
		       << ",\"selected_variant\":" << (selected.selectedRegion ? selected.selectedRegion->atlasVariantId : 0)
		       << ",\"budget_fits\":" << (selected.selectedRegion && selected.selectedRegion->supplyBudget.fits ? "true" : "false")
		       << ",\"expected_potions\":" << (selected.selectedRegion ? selected.selectedRegion->supplyBudget.expectedPotions : -1);
