...
playerbot::PlayerBotFixtureRuntime::PlayerBotFixtureRuntime(const PlayerBotTestPolicy& policy) :
	testPolicy(policy)
{
...
bool playerbot::PlayerBotFixtureRuntime::consumeNavigationStepFailure()
{
...
bool playerbot::PlayerBotFixtureRuntime::forceNavigationPlanFailure() const
{
	return forcedNavigationPlanFailuresRemaining != 0;
}

void playerbot::PlayerBotFixtureRuntime::consumeNavigationPlanFailure(bool attempted)
{
	if (attempted && forcedNavigationPlanFailuresRemaining != 0) {
		--forcedNavigationPlanFailuresRemaining;
	}
}

bool playerbot::PlayerBotFixtureRuntime::forcedStepRecoveryPending() const
{
	return testPolicy.forceRepeatedNavigationStepFailures && forcedNavigationStepFailuresRemaining != 0;
}

void playerbot::PlayerBotFixtureRuntime::resetHuntPlanningRouteFailures()
{
	huntPlanningFixtureForcedUnreachable = false;
	huntPlanningFixtureForcedNodeLimit = false;
}

bool playerbot::PlayerBotFixtureRuntime::consumeAdaptiveChallengeFixture()
{
...
bool playerbot::PlayerBotFixtureRuntime::consumeHuntPlanningCancellation()
{
	if (!testPolicy.cancelHuntPlanningAtScoreBarrier || huntPlanningFixtureCancelled) {
		return false;
	}
...
bool playerbot::PlayerBotFixtureRuntime::consumeHuntPlanningStaleRevision()
{
	if (!testPolicy.cancelHuntPlanningAtScoreBarrier || huntPlanningFixtureStaleRevisionTriggered) {
		return false;
	}
...
playerbot::PlayerBotFixtureRouteFailure playerbot::PlayerBotFixtureRuntime::consumeHuntPlanningRouteFailure()
{
	if (testPolicy.forceFirstHuntCandidateUnreachable && !huntPlanningFixtureForcedUnreachable) {
		huntPlanningFixtureForcedUnreachable = true;
		return PlayerBotFixtureRouteFailure::Unreachable;
	}
	if (testPolicy.forceSecondHuntCandidateNodeLimit && !huntPlanningFixtureForcedNodeLimit &&
	    huntPlanningFixtureForcedUnreachable) {
		huntPlanningFixtureForcedNodeLimit = true;
		return PlayerBotFixtureRouteFailure::NodeLimit;
	}
	return PlayerBotFixtureRouteFailure::None;
}

void playerbot::PlayerBotFixtureRuntime::beginDelayedInitialization()
{
	delayedInitializationPending = testPolicy.magicTrainingFixture || testPolicy.deferProgressionFixtureInitialization;
}

playerbot::PlayerBotFixtureInitialization playerbot::PlayerBotFixtureRuntime::delayedInitializationStatus(Player& player)
{
	if (!delayedInitializationPending) {
		return PlayerBotFixtureInitialization::NotPending;
	}
	if (testPolicy.deferProgressionFixtureInitialization) {
		...
		if (fixtureReady == 2) {
			...
			return PlayerBotFixtureInitialization::Cancelled;
		}
		if (fixtureReady != 1) {
			return PlayerBotFixtureInitialization::Waiting;
		}
	}
	delayedInitializationPending = false;
...
bool playerbot::PlayerBotFixtureRuntime::consumeDepotRestartCheckpoint(Player& player, DepotRestartCheckpoint checkpoint)
{
...
bool playerbot::PlayerBotFixtureRuntime::completeEquipmentPurchaseFixture(Player& player) const
{
	if (!testPolicy.equipmentPurchaseFixture) {
		return false;
	}
